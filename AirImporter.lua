...
local FAhost='flashair'
local DCIM = '/DCIM'
local Command ='command.cgi'
local URLhost = 'http://' .. FAhost
local URLhead = URLhost .. '/' .. Command .. '?'
local LsCmd = 'op=100'
...
--Get Lists of folders (top of hiearchy)
local URLListCmd=URLhead .. LsCmd.. '&DIR='
AiTasks.startAsyncTask(
	function ()
		local ProgressBar = AiProgress({
			title = 'Access to: ' .. FAhost
		})
		--confirm FlashAir Host is online
		local result,headers = AiHttp.get(URLhost)
		if headers.status ~= 200 then
			local errmsg = 'Host ' .. FAhost .. ' is not online.'
		...
		destdate[destindex]=AiDate.timeToIsoDate(datetime) --procees last item
		-- de-duplication
		local fidx = 1
		local destfolder={}
